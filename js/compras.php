...
final = {
	...
	selectorClicked : function(e){
		...
		$esto = $(this);
		metodo = $esto.data("metodo");
		Final.selectorChange(metodo);
	},
	selectorChange: function(metodo){
		Final.changeAction(metodo);
	...
	changeAction:function(metodo){
		if(metodo == "paypal"){
			Final.$form.attr("action","inc/compras_procesa_final.php");
		}else{
			Final.$form.attr("action","javascript: final.pagarConTarjeta()");
		}
		console.log(Final.$form.attr("action"));
	...
	displayError: function($elem,error){
 	...
 	pagarConTarjeta: function() {
 		Final.$form.attr("action","<?php echo $caixa_API_url; ?>");
		Final.$form.submit();
		//$.post("<?php echo $caixa_API_url; ?>",Final.$form.serialize(),function(data){console.log(data);})
	}
}
if ($("#pag-final").length ){
	final.init();
