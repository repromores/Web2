...
ORDER BY `read_date`")));
	...
	function beforeFilter() {
		$this->Auth->allow('track', 'preview', 'sendAll', 'parallelSend', 'feed');
		parent::beforeFilter();
	}
	...
	function loadcontent($input) {
		...
		foreach ($imgtags[1] as $im) {
			...
			if (!empty($paras["src"])) {
				$html_content = str_replace("[image" . $im . "]", "<img " . $htmlparas . " src=\"" . Router::url("/", true) . "thumb.php?" . http_build_query($paras) . "\" />", $html_content);
			} else {
