<div class="install index">
    ...
    <?php
        ...
        if (is_writable(APP.'config'.DS."core.php")) {
            echo '<p class="success">' . __('Your core.php is writable.', true) . '</p>';
        } else {
            $check = false;
            echo '<p class="error">' . __('Your core.php is NOT writable.', true) . '</p>';
        }

                 // config is writable
        if (is_writable(APP.'config'.DS."database.php")||!file_exists(APP.'config'.DS."database.php")) {
            echo '<p class="success">' . __('Your database.php is writable.', true) . '</p>';
        } else {
            $check = false;
            echo '<p class="error">' . __('Your database.php is NOT writable.', true) . '</p>';
        }

        if (is_writable(APP.'webroot'.DS."csv")) {
            echo '<p class="success">' . __('Your '.'webroot'.DS.'csv is writable.', true) . '</p>';
        } else {
            $check = false;
            echo '<p class="error">' . __('Your '.'webroot'.DS.'csv is NOT writable.', true) . '</p>';
        }

              if (is_writable(APP.'webroot'.DS."uploads")) {
            echo '<p class="success">' . __('Your '.'webroot'.DS.'uploads is writable.', true) . '</p>';
        } else {
            $check = false;
            echo '<p class="error">' . __('Your '.'webroot'.DS.'uploads is NOT writable.', true) . '</p>';
        }
